...
/*   Updated: 2024/07/02 20:05:12 by hsebille         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	Bitboard::isBreakableHorizontalVertical(int x, int y, int player) {
...
bool    Bitboard::isBreakable(int x, int y, int player) {
    for (int i = 0; i < 5; i++) {
		if (isBreakableHorizontalVertical(x, y, player) || isBreakableHorizontalVertical(y, x, player))
			return true;
		x++;
	}
	return (false);
...
bool	Bitboard::fiveInARowVertical(int x, int y, int player) {
    ...
    for (int i = 0; i < nbBits; i++) {
		if (i + 5 <= nbBits) {
			uint32_t five = getSelection(selection, 5, i);
			if (five == 0b11111 && !isBreakable(bitPos + i, x, player))
				return true;
		}
	}
    return false;
