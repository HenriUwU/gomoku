...
void	CustomMenu::handleAvatarSelection(const sf::RenderWindow& window) {
	if (_avatarsSprites[TOMMY].getGlobalBounds().contains(sf::Mouse::getPosition(window).x, sf::Mouse::getPosition(window).y)) {
	...
	} else if (_avatarsSprites[ALEX].getGlobalBounds().contains(sf::Mouse::getPosition(window).x, sf::Mouse::getPosition(window).y)) {
		if (playerOneAvatar == NOAVATAR)
